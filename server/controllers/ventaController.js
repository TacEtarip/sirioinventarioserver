...
export const getLastTenVentas = async (req, res) => {
  try {
    ...
    const ventas = await Venta.find({ vendedor: searchRegex })
      .sort({ date: -1 })
      .limit(20);
    res.json(ventas);
  } catch (error) {
...
export const eliminarItemScVenta = async (req, res) => {
  try {
    ...
    const newTotalPrice =
      Math.round(
        (preSearch.totalPrice - req.body.totalPriceSC + Number.EPSILON) * 100
      ) / 100;
    ...
    for (const item of preSearch.itemsVendidos) {
      if (req.body.itemCodigo === item.codigo) {
        itemNewTotalPrice =
          Math.round(
            (item.totalPrice - req.body.totalPriceSC + Number.EPSILON) * 100
          ) / 100;
    ...
    const result = await Venta.findOneAndUpdate(
      {
        ...
        itemsVendidos: { $elemMatch: { codigo: req.body.itemCodigo } },
      },
      {
        $pull: {
        ...
        $set: {
          ...
          totalPriceNoIGV: newTotalPriceNoIGV,
        },
      },
...
export const eliminarItemVenta = async (req, res) => {
  try {
    const preSearch = await Venta.findOne({ codigo: req.body.codigo });
    const indexEx = preSearch.itemsVendidos.findIndex(
    ...
    const newTotalPrice =
      Math.round(
        (preSearch.totalPrice - req.body.totalItemPrice + Number.EPSILON) * 100
      ) / 100;
    const newTotalPriceNoIGV = getNoIGV_Price(newTotalPrice);
    const result = await Venta.findOneAndUpdate(
      { codigo: req.body.codigo },
...
export const getVentasEjecutadasTest = async (req, res) => {
  try {
    ...
    const result = await Venta.find({
      estado: "ejecutada",
      "documento.name": { $regex: searchRegex },
    })
      .skip(parseInt(0))
      .limit(10)
      .sort({ date: -1 });
...
export const getRUC = async (req, res) => {
  try {
    const result = await axios.post("https://api.migo.pe/api/v1/ruc", {
      token: tokenSunat,
      ruc: req.params.ruc,
    });
    res.json(result.data);
...
export const createExcel = async (req, res) => {
  try {
    ...
    const buffer = await createExcelReport(
      "Ventas",
      "Reporte De Ventas",
      ["Fecha", "Codigo", "Para", "Vendido Por", "Precio No IGV", "Precio"],
      ventas
    );
    res.writeHead(200, {
      "Content-Type":
    ...
    const result = await Venta.aggregate([
      ...
      { $match: { "itemsVendidos.codigo": req.params.codigoItem } },
      {
        $group: {
          ...
          totalVenta: {
            $sum: {
              $multiply: ["$itemsVendidos.priceIGV", "$itemsVendidos.cantidad"],
            },
          },
          totalGasto: {
            $sum: {
              $multiply: [
                "$itemsVendidos.priceCosto",
...
export const getVentasPorDiaMes = async (req, res) => {
  try {
          ...
          gastosVenta: {
            $sum: {
              $map: {
                ...
                in: {
                  $multiply: [
                    "$$itemVendido.cantidad",
                    "$$itemVendido.priceCosto",
                  ],
                },
              },
            },
    ...
    for (let index = 0; index < dias; index++) {
      const vpd = ventasPorDia[currentIndexVentasDia]
        ? ventasPorDia[currentIndexVentasDia]._id
        : -1;
      if (vpd === index + 1) {
