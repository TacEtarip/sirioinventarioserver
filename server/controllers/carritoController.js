...
export const addItemToCarrito = async (req, res) => {
	...
	try {
		...
		const item = await User.findOne(req.body.codigo);
		...
		if (user.carrito === 'not') {
			...
			await User.findOneAndUpdate({ username: req.user.aud.split(' ')[0] }, { carrito: req.ventaGenerada.codigo });
		}
		else {
						...
						req.ventaGenerada = await Venta.findOneAndUpdate({ codigo: user.carrito, 'itemsVendidos.codigo': item.codigo },
							{
								...
								$set: {
									...
									'itemsVendidos.$.priceCosto': item.priceCosto,
						...
						req.ventaGenerada = await Venta.findOneAndUpdate({ codigo: user.carrito, 'itemsVendidos.codigo': item.codigo },
							{
								...
								$pull: {
									'itemsVendidos.$.cantidadSC': {
										name: inOrder.name, nameSecond: inOrder.nameSecond || undefined,
										cantidadDisponible: ventaActiva.itemsVendidos[itemExiste].cantidadSC[cantidadSCE].cantidadDisponible,
										cantidadVenta: ventaActiva.itemsVendidos[itemExiste].cantidadSC[cantidadSCE].cantidadVenta,
									},
								},
								$push: {
									'itemsVendidos.$.cantidadSC': {
...
const generarCodigo = async () => {
	try {
		const count = await Venta.countDocuments({ tipoVendedor: 'low' });
		let preCod = 'CC01-000000';
