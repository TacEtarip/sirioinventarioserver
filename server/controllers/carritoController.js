...
export const addItemToCarrito = async (req, res) => {
	...
	try {
		...
		if (user.carrito === 'not') {
			...
			await User.findOneAndUpdate({ username: req.user.aud.split(' ')[0] }, { carrito: req.ventaGenerada.codigo }, { useFindAndModify: false, new: true });
		}
		else {
...
export const getCarrito = async (req, res) => {
	...
	try {
		...
		// Comprobar Carrito
		const tipoDeAlertas = [{ cod: 0, mensaje: 'Item Eliminado' }, { cod: 1, mensaje: 'Cambio de precio' }];
		const alerts = [];
		...
		for (let index = 0; index < carrito.length; index++) {
			...
			if (itemsEnVenta[indexOfItems].deleted) {
				if (itemDeletedCod !== itemsEnVenta[indexOfItems].codigo) {
					...
					alerts.push({ ...tipoDeAlertas[0],
						item_cod: itemsEnVenta[indexOfItems].codigo,
						item_name: itemsEnVenta[indexOfItems].name });

					await Venta.findOneAndUpdate({ codigo: req.carrito_codigo },
						{
							$inc: {
								totalPrice: -(carrito[index].cantidad * carrito[index].priceIGV),
								totalPriceNoIGV: -(carrito[index].cantidad * carrito[index].priceNoIGV),
							},
				...
				(itemCambioPrecioCod !== itemsEnVenta[indexOfItems].codigo)) {
					itemCambioPrecioCod = itemsEnVenta[indexOfItems].codigo;
					alerts.push({ ...tipoDeAlertas[1],
						item_cod: itemsEnVenta[indexOfItems].codigo,
						item_name: itemsEnVenta[indexOfItems].name });

					const totalPrecioNuevo = itemsEnVenta[indexOfItems].priceIGV * carrito[index].cantidad;
					const totalPrecioNuevoNoIGV = itemsEnVenta[indexOfItems].priceNoIGV * carrito[index].cantidad;
					await Venta.findOneAndUpdate({ codigo: req.carrito_codigo, 'itemsVendidos.codigo': itemsEnVenta[indexOfItems].codigo },
						{
							$inc: {
								totalPrice: (totalPrecioNuevo - carrito[index].totalPrice),
								totalPriceNoIGV: (totalPrecioNuevoNoIGV - carrito[index].totalPriceNoIGV),
							},
							$set: {
								'itemsVendidos.$.totalPrice': totalPrecioNuevo,
								'itemsVendidos.$.totalPriceNoIGV': totalPrecioNuevoNoIGV,
								'itemsVendidos.$.priceIGV': itemsEnVenta[indexOfItems].priceIGV,
								'itemsVendidos.$.priceNoIGV': itemsEnVenta[indexOfItems].priceNoIGV,
							},
						}, { useFindAndModify: false, new: true });
				}
				...
				carrito[index].totalPrice = itemsEnVenta[indexOfItems].priceIGV * carrito[index].cantidad;
				carrito[index].totalPriceNoIGV = itemsEnVenta[indexOfItems].priceNoIGV * carrito[index].cantidad;

				if (itemsEnVenta[indexOfItems].subConteo) {

					const carritoToLoop = carrito[index];
					const cantidadDisponible =
						itemsEnVenta[indexOfItems].subConteo
							.order.find(o => o.name === carritoToLoop.cantidadSC.name &&
								o.nameSecond === carrito[index].cantidadSC.nameSecond);


					finalArrayCarrito.push({ ...carrito[index], cantidadDisponible: cantidadDisponible.cantidad, codVent: req.carrito_codigo });
				}
				else {
					finalArrayCarrito.push({ ...carrito[index], cantidadDisponible: itemsEnVenta[indexOfItems].cantidad, codVent: req.carrito_codigo });
				}
			}
		...
		await session.commitTransaction();
		session.endSession();
		res.json({ carrito: finalArrayCarrito, cod_carrito: req.carrito_codigo, estado_carrito: req.estadoCarrito, alerts: alerts });
	}
	catch (error) {
		await session.abortTransaction();
		session.endSession();
...
export const removeProductoDeCarrito = async (req, res) => {
	try {
		...
		if (carritoActParsed .cantidadSC &&
			(carritoActParsed.cantidad - carritoActParsed.cantidadSC.cantidadVenta) > 0) {
			newCarrito = await Venta.findOneAndUpdate({ codigo: req.body.codVent, 'itemsVendidos.codigo': req.body.codigo },
				{
					$inc: {
						totalPrice: -(carritoActParsed.cantidadSC.cantidadVenta * carritoActParsed.priceIGV),
						totalPriceNoIGV: -(carritoActParsed.cantidadSC.cantidadVenta * carritoActParsed.priceNoIGV),
						'itemsVendidos.$.cantidad': -carritoActParsed.cantidadSC.cantidadVenta,
						'itemsVendidos.$.totalPrice': -(carritoActParsed.cantidadSC.cantidadVenta * carritoActParsed.priceIGV),
						'itemsVendidos.$.totalPriceNoIGV': -(carritoActParsed.cantidadSC.cantidadVenta * carritoActParsed.priceNoIGV),
					},
					$pull: { 'itemsVendidos.$.cantidadSC': { name: carritoActParsed.cantidadSC.name, nameSecond: carritoActParsed.cantidadSC.nameSecond } },
				}, { useFindAndModify: false, new: true });
		}
		else {
			newCarrito = await Venta.findOneAndUpdate({ codigo: req.body.codVent },
				{
					$inc: {
						totalPrice: -(carritoActParsed.cantidad * carritoActParsed.priceIGV),
						totalPriceNoIGV: -(carritoActParsed.cantidad * carritoActParsed.priceNoIGV),
					},
