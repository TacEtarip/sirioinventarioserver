	...
	try {
		const result =
                        await Cotizacion.aggregate([
                        	...
                        	{ $match: { $or: [{ 'cantidadSC.cantidadVenta': { $gt: 0 } }, { cantidadSC: null }] } },
                        ]);
		res.json(result);
	}
	catch (error) {
...
export const agregarItemCoti = async (req, res) => {
	try {
		const preInfo = await Cotizacion.aggregate([{ $match: { codigo: req.body.codigoCoti } }, { $unwind: '$itemsVendidos' },
		...
		if (preInfo.length !== 0) {
			...
			const ventaAct = await Cotizacion.findOneAndUpdate(
				{ codigo: req.body.codigoCoti, itemsVendidos: { $elemMatch: { codigo: req.body.itemVendido.codigo } } },
		...
		const ventaActT = await Cotizacion.findOneAndUpdate({ codigo: req.body.codigoCoti },
...
export const eliminarItemCoti = async (req, res) => {
	try {
		const result =
        await Cotizacion.findOneAndUpdate({ codigo: req.body.codigo },
        	{ $inc: { totalPrice: (req.body.totalPrice) * -1, totalPriceNoIGV:  (req.body.totalPriceNoIGV) * -1 },
...
export const eliminarCoti = async (req, res) => {
	try {
		await Cotizacion.findOneAndUpdate({ codigo: req.body.codigo }, { estado: 'eliminada' });
