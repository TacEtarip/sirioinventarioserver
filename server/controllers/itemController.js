...
export const ventaSimpleItemUpdate = async (req, res, next) => {
  ...
  try {
    ...
    const lastResort = await Item.findOne({
  ...
  try {
    ...
    const result = await Venta.findByIdAndUpdate(
      req.ventResult._id,
      {
        ...
        guia_link: req.sunat_guia.enlace_del_pdf,
...
export const cantidadUpdate = async (req, res) => {
  try {
    ...
    const result = await Item.findOneAndUpdate(
      ...
      {
        $inc: {
          cantidad: req.body.tipo ? req.body.cantidad : req.body.cantidad * -1,
        },
        $push: { variaciones: variacion },
      },
      { new: true, useFindAndModify: false }
...
export const subCantidadUpdate = async (req, res) => {
  try {
    ...
    for (const sc of itemOriginal.subConteo.order) {
      const indexToMDF = req.body.subConteo.order.findIndex(
        (v) => v.name === sc.name && v.nameSecond === sc.nameSecond
      );
      if (indexToMDF !== -1) {
    ...
    for (const sc of req.body.subConteo.order) {
      const indexToMDF = itemOriginal.subConteo.order.findIndex(
        (v) => v.name === sc.name && v.nameSecond === sc.nameSecond
      );
      const cantidadOrg =
        indexToMDF > -1 ? itemOriginal.subConteo.order[indexToMDF].cantidad : 0;
      if (indexToMDF > -1) {
        if (req.body.tipo) {
        ...
        itemOriginal.subConteo.order.push({
          cantidad: sc.cantidad,
          name: sc.name,
          nameSecond: sc.nameSecond || undefined,
        });
      }
      variacion.cantidadSC.push({
        name: sc.name,
        nameSecond: sc.nameSecond,
        cantidadDisponible: cantidadOrg,
        cantidadVenta: sc.cantidad,
      });
    }
...
export const updateItemsSubTipo = async (req, res) => {
  try {
    const result = await Item.updateMany(
      { subTipo: req.body.antiguoSubName },
      { subTipo: req.body.newSubName },
      { new: true, useFindAndModify: false }
    );
    res.json(result);
  } catch (error) {
...
export const deleteItemsSubTipo = async (req, res) => {
  try {
    const result = await Item.updateMany(
      { subTipo: req.params.subTipoName },
      { deleted: true }
    );
    res.json(result);
  } catch (error) {
...
export const filtrarTopFive = async (req, res) => {
  try {
    ...
    for (let index = 0; index < gananciaPorItemFive.length; index++) {
      const newName = await Item.findOne({
        codigo: gananciaPorItemFive[index].name,
      });
...
export const optenerVariacionPosneg = async (req, res) => {
  try {
    ...
    res.json({
      gastosInventario:
        variacionPositiva[0].totalVarPos -
        ((variacionNegativaQuitar[0]
          ? variacionNegativaQuitar[0].totalVarQuitar
          : 0) +
          (variacionPosivaAnular[0]
            ? variacionPosivaAnular[0].totalVarAnular
            : 0)),
      gananciasInventario:
        variacionNegativa[0].totalVarNeg -
        ((variacionNegativaQuitar[0]
          ? variacionNegativaQuitar[0].totalVarQuitar
          : 0) +
          (variacionPosivaAnular[0]
...
export const optenerVentasPotenciales = async (req, res) => {
  try {
    ...
    res.json({
      ...
      costoPropioTotalAprox: gastosCalculado[0].totalPotencial,
      gananciaAprox:
  ...
  try {
    ...
    res.json({
      gananciaEnVentasFueraTienda:
        ...
        gastosItemsFueraTienda[0].costoTotalPropio,
      gananciaEnVentas:
        gastosItemsDeTiendaTotales[0].ingresoTotal -
        gastosItemsDeTiendaTotales[0].costoTotalPropio,
...
export const getClientesConMasCompras = async (req, res) => {
  try {
    ...
    res.json({
      cliente: mejoresClientes[0]._id.name,
...
export const deleteCaracteristica = async (req, res) => {
  try {
    const result = await Item.findOneAndUpdate(
      { codigo: req.body.itemCod },
      { $pull: { caracteristicas: { $in: req.body.deleteArray } } },
      { useFindAndModify: false, new: true }
    );
    res.json(result);
  } catch (error) {
...
export const addCaracteristica = async (req, res) => {
  try {
    const result = await Item.findOneAndUpdate(
      { codigo: req.body.itemCod },
      { $push: { caracteristicas: req.body.newCaracteristica } },
      { useFindAndModify: false, new: true }
    );
    res.json(result);
  } catch (error) {
...
export const reOrderItems = async (req, res) => {
  ...
  session.startTransaction();
  try {
    const preBusqueda = await Item.countDocuments({
      deleted: false,
      subTipo: req.body.itemReOrder[0].subTipo,
    ...
    for (let index = 0; index < req.body.itemReOrder.length; index++) {
      await Item.findOneAndUpdate(
        { codigo: req.body.itemReOrder[index].codigo },
        { orderNumber: index },
        { useFindAndModify: false }
      );
    }
...
export const getItemsLowStock = async (req, res) => {
  try {
    const sinStockItems = await Item.find({
      deleted: false,
      $and: [{ cantidad: { $lt: 5 } }, { cantidad: { $gt: 0 } }],
    });
...
export const getGICofItem = async (req, res) => {
  try {
    ...
    res.json({
      itemVendidos:
        cantidadHistorica[0].totalCantidadVenta -
        cantidadHistorica[0].totalCantidadAnular,
      cantidadHistorica:
        cantidadHistorica[0].totalCantidadNI +
        cantidadHistorica[0].totalCantidadAgregar -
        cantidadHistorica[0].totalCantidadQuitar,
      totalCosto:
        itemGIC[0].totalTrue -
        ((variacionNegativaQuitar[0]
          ? variacionNegativaQuitar[0].totalVarQuitar
...
export const addCategoryToALL = async (req, res) => {
  try {
    const result = await Item.updateMany(
      {},
