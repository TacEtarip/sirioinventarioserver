...
export const ps = passport.use(new GoogleStrategy({
...
const registerWithGoogleMock = async (googleUser) => {
    try {
        ...
        const newUser = new User( { username: 'testGoogle', type: 'low', displayName: 'TestGoogle', email: googleUser.email,
                                    nombre: googleUser.given_name, apellido: googleUser.family_name, google: true, 
                                    googleCod:  googleUser.sub, dni: '763559537' } );
...
export const loginUserToken = async (req, res, next) => {
    jwt.verify(req.body.loginToken, config[process.env.NODE_ENV].jwtLogin, (err, decode) => {
        if (err) {
...
export const doTheLogin = async (req, res) => {
    try {
        ...
        return res.json({displayName: user.displayName, username: user.username, success:true, message: 'Success', type: user.type, 
...
export const userExits = async (req, res) => {
    try {
        const result = await User.exists({username: req.params.username.toLowerCase()});
...
export const getLoginInfoFromToken = async (req, res) => {
    jwt.verify(req.body.loginToken, config[process.env.NODE_ENV].jwtGoogleLogin, (err, decode) => {
        if (err) {
            if (err.name === 'TokenExpiredError') {
                return res.status(400).json({error: 'Token Expirado'});
            }
...
export const login = async (req, res) => {
    try {
        ...
        if(! await result.comparePassword(req.body.password, result.hashPassword)){
            return res.status(400).json({username: req.body.username, success:false ,message: 'Authenticacion failed. Incorrect Password!', token: null});
        } else {
            return res.json({displayName: result.displayName, username: result.username, success:true, message: 'Success', type: result.type, 
                    token: jwt.sign({ aud: result.username + ' ' + result.type, 
                    _id: result.id}, config[process.env.NODE_ENV].jwtKey)});
        }
    } catch (error) {
        throw error;
    }
...
export const getUserInfo = async (req, res) => {
    try {
        const result = await User.findOne({ username: req.params.username.toLowerCase() });
...
export const agregarDireccion = async (req, res) => {
    try {
        ...
        await User.findOneAndUpdate(
            { _id: req.body.id }, 
            { dirOne: req.body.dirOne, dirTwo: req.body.dirTwo || '', reference: req.body.reference, city: 'Trujillo, La Libertad. Perú' }, 
            { useFindAndModify: false, new: true 
            });
        result.hashPassword = undefined;
        res.json(result);
...
export const confirmarContrasena = async (req, res) => {
    try {
        ...
        if (! await result.comparePassword(req.body.oldPassword, result.hashPassword)) {
...
export const cambiarContrasena = async (req, res) => {
    try {
        const result = await User.findById(req.body.id);
        if (! await result.comparePassword(req.body.passwordOld, result.hashPassword)) {
            return res.json({ changed: false });
        } 
        const newHash = await bcrypt.hash(req.body.password, 10);
        await User.findByIdAndUpdate(req.body.id, { hashPassword: newHash }, { new: true, useFindAndModify: false });
