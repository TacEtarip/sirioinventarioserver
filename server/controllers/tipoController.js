...
export const getSiteMapLinks = async (req, res) => {
  try {
    ...
    const siteMapsArray = [
      ...
      {
        url: "servicio-al-cliente/contactanos",
        priority: 0.8,
        changefreq: "monthly",
      },
      { url: "carrito", priority: 0.7, changefreq: "monthly" },
      {
        url: "servicio-al-cliente/terminos-y-condiciones",
        priority: 0.3,
        changefreq: "monthly",
      },
      {
        url: "servicio-al-cliente/politica-de-privacidad",
        priority: 0.3,
        changefreq: "monthly",
      },
      {
        url: "servicio-al-cliente/politica-de-devolucion",
        priority: 0.3,
        changefreq: "monthly",
      },
      {
        url: "servicio-al-cliente/politica-de-envio",
        priority: 0.3,
        changefreq: "monthly",
      },
    ...
    for (let index = 0; index < getTipos.length; index++) {
      siteMapsArray.push({
        url: "store/categorias/" + getTipos[index].name,
        priority: 0.7,
        changefreq: "daily",
      });
    ...
    for (let index = 0; index < getItems.length; index++) {
      siteMapsArray.push({
        url:
          ...
          getItems[index].tipo +
          "/" +
          getItems[index].subTipo +
          "/" +
          getItems[index].codigo,
        priority: 0.5,
        changefreq: "daily",
      });
    }
...
export const getSubTipos = async (req, res) => {
  try {
    const result = await Tipo.findOne({ name: req.params.tipoCod }).select(
      "subTipo -_id"
    );
    return res.json(result);
  } catch (error) {
...
export const updateTipo = async (req, res, next) => {
  try {
    ...
    const newTipo = await Tipo.findOneAndUpdate(
      { codigo: req.body.codigo },
      { name: req.body.tipoName },
...
export const uploadPhotoNameCat = async (req, res) => {
  try {
    const result = await Tipo.findOneAndUpdate(
      { codigo: req.params.codigo },
      { link: req.fileName },
      { new: true, useFindAndModify: false }
    );
...
export const deleteTipo = async (req, res, next) => {
  try {
    const tipoDeleted = await Tipo.findOneAndUpdate(
      { codigo: req.params.codigo },
      { deleted: true },
      { new: true, useFindAndModify: false }
    );
...
export const reOrderTipo = async (req, res) => {
  ...
  try {
    ...
    for (let index = 0; index < req.body.tiposReOrder.length; index++) {
      await Tipo.findOneAndUpdate(
        { codigo: req.body.tiposReOrder[index].codigo },
        { order: index },
...
export const cambiarItemCarpeta = async (req, res) => {
  ...
  try {
    const result = await Item.findOneAndUpdate(
      { codigo: req.body.codigo },
      { subTipo: req.body.newSubTipo, tipo: req.body.newTipo, orderNumber: -2 },
      { useFindAndModify: false, new: true }
    );
