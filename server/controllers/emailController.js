...
export const sendTestEmail = async (req, res) => {
  try {
    const html = await readFilePromise(
      path.resolve(__dirname, "../lib/email.html"),
      { encoding: "utf8" }
    );
...
export const sendMensajeEmail = async (req, res) => {
  try {
    const html = await readFilePromise(
      path.resolve("server/lib/emailContacto.html"),
      { encoding: "utf8" }
    );
    ...
    const htmlReplacedSecondPhase = htmlReplaced.replace(
      "#replaceWithLinkTwo",
      req.body.mensaje
    );
...
export const sendConfirmationEmail = async (req, res) => {
  try {
    const html = await readFilePromise(path.resolve("server/lib/email.html"), {
      encoding: "utf8",
    });
