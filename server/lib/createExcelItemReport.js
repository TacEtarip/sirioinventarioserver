...
export const createExcelItemReport = async (item) => {
  try {
    ...
    const current = createTableCtnWS(
      ws,
      item,
      5,
      styleNormal,
    ...
    const currentSecond = createTableCtnWS2(
      ws2,
      item,
      5,
      styleNormal,
...
const createHeaders = (wb, ws, ws2, ws3, name) => {
  ...
  ws.cell(2, 2, 2, 6, true).string(`Gastos | ${name}`).style(styleHeader);

  ws2.cell(2, 2, 2, 6, true).string(`Ganancias | ${name}`).style(styleHeader);

  ws3.cell(2, 2, 2, 6, true).string(`Balance | ${name}`).style(styleHeader);
...
const createSubHeaderWS = (wb, ws, ws2) => {
  ...
  for (const subH of arrayOfSubHeaders) {
    ...
    ws2.cell(4, startColumn).string(subH).style(styleSub);
    startColumn++;
  }
...
const createTableCtnWS = (ws, item, startRow, sn, sns, cDesv) => {
  ...
  if (current >= item.variaciones.length) {
    ...
    ws.cell(startRow, 4).string(currentVar.cantidad.toString()).style(style);
    ...
    ws.cell(startRow, 5).string(subC).style(style);

    const costo = currentVar.costoVar;

    ws.cell(startRow, 6).number(costo).style(style);
    ...
    ws.cell(startRow, 3).string(comentariParse).style(style);
