...
export const createExcelReport = async (wsName, excelHeader, arrayOfSubHeaders, arrayVentas) => {
...
const createTableCtn = (ws, ventas, startRow, sn, sns) => {
    ...
    if (current >= ventas.length) {
        ...
        ws.cell(startRow, 3)
        .string(ventas[current].codigo)
        .style(style);

        let doc = '';
        if (ventas[current].documento.name) {
            doc = ventas[current].documento.name + ' | ' + ventas[current].documento.codigo;
        }
        ws.cell(startRow, 4)
        .string(doc)
        .style(style);
