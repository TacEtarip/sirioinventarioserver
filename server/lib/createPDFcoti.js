	...
	content: [
		{
			...
			table: {
				...
				body: [
                    ...
                    [{image: '',  width: 100, border: [false, false, false, false], 
					style: 'headerTitle'}, 
					{text: [
					    { text: '', fontSize: 18, bold: true },
					    { text: '\nSirio Dinar - RUC: 20605746587', color: 'darkblue', fontSize: 12  },
					    { text: '\ninventario.siriodinar.com', color: 'orange', fontSize: 12  }
					    ],  
					rowSpan: 2, alignment: 'center', style: 'headerTitlePrinc',
					border: [true, true, true, false]} ],
					[{text: 'Jr. Pardo y Aliaga Nro. 207. Trujillo - Perú \n +51 977 426 349 | +51 922 412 404',  
					width: 100, height: 100, border: [false, false, false, false], 
					style: 'headerTitle'} ]
...

		{
		  ...
		  style: 'tableEnviar',
			table: {
			    ...
			    widths: [165, 331],
				body: [
					[{text: 'Enviar a:', fillColor: '#005087', bold: true, color: '#EAF6FF', style: 'tableMargin', },
					{text: 'Observaciones:', fillColor: '#005087', bold: true, color: '#EAF6FF', style: 'tableMargin'}],
          [{text: '',  border: [true, true, true, false], 
					style: 'tableMargin', }, {text: 'RR',  
					border: [true, true, true, false], 
					style: 'tableMargin'} ],
...

		{
			...
			table: {
			    ...
			    widths: [15, 97, 35, 70, 120, 35, 35, 35 ],
				  body: [
					[{text: 'Nº', style: 'tableMarginSH'}, {text: 'IMAGÉN', style: 'tableMarginSH'}, 
					{text: 'UDM', style: 'tableMarginSH'}, {text: 'NOMBRE', style: 'tableMarginSH'},
					{text: 'DESCRIPCIÓN', style: 'tableMarginSH'}, {text: 'CANT.', style: 'tableMarginSH'}, 
					{text: 'P/CU', style: 'tableMarginSH'}, {text: 'TOTAL', style: 'tableMarginSH'}],
		...
		
		{
		    ...
		    table: {
		        ...
		        body: [
		            [ 
		            {text: 'Sub Total:', border: [true, false, false, true], fillColor: '#E87100', color: 'white', style: 'totalesMargin'},
		            {text: 'S/ 111.00', border: [true, false, true, true], alignment: 'right', style: 'totalesMargin' },
		            ],
		            [ 
		            {text: 'IGV:', color: 'grey', style: 'totalesMargin' },
		            {text: 'S/ 111.00', alignment: 'right', color: 'grey', style: 'totalesMargin' },
		            ],
		            [ 
		            ...
		            [ 
		            {text: 'Total:', fillColor: '#005087', color: 'white', style: 'totalesMargin' }, 
...
export const createDocument = (buffer, coti, extra, arrayBuffers, imagen) => {
    ...
    cotiDoc.content[4].table.body[0][1].text = 'S/ ' + coti.totalPriceNoIGV.toFixed(2);
    cotiDoc.content[4].table.body[1][1].text = 'S/ ' + (Math.round((coti.totalPrice - coti.totalPriceNoIGV) * 100) / 100).toFixed(2);
    cotiDoc.content[4].table.body[2][1].text = 'S/ ' + envioR.toFixed(2);
    cotiDoc.content[4].table.body[3][1].text = 'S/ ' + otroR.toFixed(2);
    cotiDoc.content[4].table.body[4][1].text = 'S/ ' + ((extra.otro || 0) + (extra.envio || 0) + coti.totalPrice).toFixed(2);
    const doc = printer.createPdfKitDocument(cotiDoc);
    return doc;
};

const preInfoDocCoti =  (buffer, coti, extra) => {
  const d2 = DateTime.fromJSDate(coti.date).toFormat('c LL yyyy').toLocaleString();
  ...
  cotiDoc.content[1].table.body[1][0].text = 
  `${coti.documento.name} | ${coti.documento.codigo}${coti.cliente_email ? `\nemail: ${coti.cliente_email}` : ''}${coti.celular_cliente === '+51' ? '' : `\nCelular:${coti.celular_cliente}`}`;
};
...
const agregarItems = (coti, arrayBuffers, imagen) => {
  ...
  coti.itemsVendidos.forEach((item, index) => {
    ...
    if (item.cantidadSC && item.cantidadSC.length > 0) {
      ...
      for (let index = 0; index < item.cantidadSC.length; index++) {
          if (item.cantidadSC[index].cantidadVenta > 0) {
              descrpString = descrpString + 
              ` ${item.cantidadSC[index].name} ${item.cantidadSC[index].nameSecond ? `- ${item.cantidadSC[index].nameSecond}: ` : ': '}` +
               item.cantidadSC[index].cantidadVenta + `${item.cantidadSC.length + 1 < index ? ',' : '.'}` ;
          }
      }
    }
    if (imagen) {
