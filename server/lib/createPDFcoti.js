	...
	coti.itemsVendidos.forEach((item, index) => {
		...
		if (item.cantidadSC && item.cantidadSC.length > 0) {
			...
			for (let indexS = 0; indexS < item.cantidadSC.length; indexS++) {
              ...
              ` ${item.cantidadSC[indexS].name} ${item.cantidadSC[indexS].nameSecond ? `- ${item.cantidadSC[indexS].nameSecond}: ` : ': '}` +
               item.cantidadSC[indexS].cantidadVenta + `${item.cantidadSC.length + 1 < indexS ? ',' : '.'}` ;
				}
			}
		}
		if (imagen) {
